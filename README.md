...
1. In the [constructor](https://www.php.net/manual/en/language.oop5.decon.php) of the `Player` class;
    ...
    - Add these cards to the `cards` array property of the player 
1. Save the instance of the entire `Blackjack`object in the session (you're gonna need it)
...
1. Use the class' methods to react to these actions.
    - `hit` should add a card to the player (to the `cards` property). If this brings him above 21, set the `lost` property to `true`. You can access the deck by passing it as a parameter with the getDeck method of your blackjack class.
